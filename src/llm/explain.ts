...
function redactFigures(text: string): string {
  ...
  // every real occurrence — there's no boundary marker to lean on. The one
  // known collision at this length is "구원" (salvation: 구=9 + 원), which the
  // replacer below special-cases rather than weakening the match generally.
  ...
  return text
    .replace(digitRun, ' (금액) ')
    .replace(koreanNumeralPhrase, (whole, numeral, unit) =>
      numeral === '구' && unit === '원' ? whole : ' (금액) ',
    )
